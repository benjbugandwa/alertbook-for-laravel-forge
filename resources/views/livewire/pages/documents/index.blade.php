                ...
                <td class="px-4 py-3">
                    <div class="flex items-center gap-2">
                        ...
                        @if ($this->canEditOrAdd() && $doc->uploaded_by === auth()->id())
                            ...
                            {{-- Delete --}}
                            <button type="button"
                                class="h-9 w-9 inline-flex items-center justify-center rounded-lg border border-red-200 hover:bg-red-50 text-red-600"
                                wire:click="delete('{{ $doc->id }}')"
                                wire:confirm="Supprimer définitivement ce document et son fichier ?"
                                title="Supprimer">
                                <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M6 7h12m-10 0 1 13h6l1-13m-6 0V4h4v3" />
                                </svg>
                            </button>
                        @endif
                    </div>
